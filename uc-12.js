...
console.log(
  "7d)Find 1st occurences with full day salary: " +
    mapDayWithWageArr.find(findFullTimeFirstOccurence)
);
...
console.log(
  "7e)IS Find every occurences with full day salary: " +
    fullTime.every(isAllFullTimeWage)
);
...
console.log(
  "7g)Number of days employee worked : " +
...
console.log("***UC8 - Using Map***");
{
  ...
  while (
    ...
    empDailyWageMap.set(
      totalWorkingDays,
      calculateDailyWage(getWorkinghours(empCheck))
    );
  }
  ...
  empDailyHrmap.forEach((value, key, map) => {
    if (value == 8) fullWorkingDay.push(key);
    else if (value == 4) partWorkingDay.push(key);
    else absentDays.push(key);
  });
...
//UC-10 Use objects to store data
  ...
  while (
    ...
    empDailyHrsandWage.push({
      ...
      hrsWorked:getWorkinghours(empCheck),
      Wage:calculateDailyWage(getWorkinghours(empCheck)),
      toString(){
